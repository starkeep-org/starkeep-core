...
Starkeep is designed so that data is easily shared across apps running on the system. This is accomplished by identifying every file by its lowercase extension and organizing extensions into a platform-owned set of categories (Images, Videos, Documents, …). The `extension → category` map is hardcoded in core, so the semantics of each file type are clearly defined and different apps can interoperate on the same items. Apps declare the exact extensions they handle (and may only declare known ones); files with unmapped or absent extensions fall into a terminal `other` category that only the built-in Drive (User-Data-Owner) can see.
