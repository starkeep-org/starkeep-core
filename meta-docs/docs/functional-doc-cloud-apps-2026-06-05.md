# Cloud apps — Functional Review (2026-06-05, revised 2026-06-10)

> **Revision note (2026-06-10).** Revised after commits `345ec13` (per-app HMAC credentials + cloud-mode `@starkeep/app-client`), `10ce75b` (cloud-side `/app-data/*` route added, Pulumi passphrase ensure-if-missing), and `9e88958` (removed `owner_id` from `shared.records`). Part 1 reflects the current state; Part 2 captures the bugs introduced in the same pass — most notably an unreachable `/app-data/*` route and an HMAC-vs-gateway-JWT mismatch.
...
- **Per-app cloud identity.** Each app's authority on the cloud is one IAM role plus one PG role plus a small set of grant rows plus a per-app HMAC secret. The IAM/PG/grant trio is what AWS and DSQL enforce; the HMAC secret is what the broker uses to prove the *calling* app is the one it claims to be when forwarding a request. All four are minted at install and dropped at uninstall.
...
1. **`put_app_creds_parameter`** (as Manager). Writes the per-app HMAC secret to SSM SecureString at `/${stackPrefix}/app-creds/${appId}`. The secret is sourced from the local creds file at `~/.starkeep/app-creds/${appId}.json` (written by admin-web at local install) so the local sync supervisor and the cloud broker both sign/verify under the same key. If the local file is absent, `ensureLocalHmacSecret` mints a fresh 32-byte hex string for SSM (see Part 2 — this leaves the local side without the same secret).
2. **`create_iam_role`** (as Manager). Mints `<stackPrefix>-app-<appId>-role` with one of three permissions boundaries:
   ...
   The role's inline runtime policy is built from `fileAccess` + `appSpecificSyncable` to cap S3 prefixes (`shared/<category>/...` for granted categories, `apps/<appId>/...` for app-private), DSQL connect (`sts:DbConnect` on the app's PG role), and a scoped `ssm:GetParameter` on `/${stackPrefix}/app-creds/${appId}` plus a `kms:Decrypt` gated by `kms:ViaService=ssm.*.amazonaws.com` so the app's own Lambda (cloud-mode `@starkeep/app-client`) can fetch its HMAC secret at runtime. The cloud-data-server's role gets a broader `ssm:GetParameter` on `/${stackPrefix}/app-creds/*` so the verifier can load any app's secret on demand.
3. **`attach_temp_install_ddl_policy`** (as Manager). Attaches `<stackPrefix>-temp-install-ddl-<appId>` to the `install-ddl-role`, granting the DSQL admin verbs needed to run this app's DDL.
4. **`run_dsql_ddl`** (as `install-ddl-role`). Connects to DSQL as the `admin` PG role with a `DbConnectAdmin` token and runs the per-app DDL:
...
5. **`detach_temp_install_ddl_policy`** (as Manager). Symmetric to step 3 — the broad DSQL admin verbs are gone the moment the DDL transaction ends successfully.
6. **`put_s3_keep_file`** (as the app role). Writes a marker object under `apps/<appId>/.keep`. The point is to confirm the runtime S3 path works under the freshly-minted credentials before any user-data interaction is attempted.
7. **`attach_temp_install_infra_policy`** (as Manager). Skipped if the manifest has no `zipBuffer` and `compute.enabled: false`. Attaches the per-app temp policy to `install-infra-role` granting the Lambda / API Gateway / artifacts-bucket verbs needed for this app's Pulumi up.
8. **`upload_bundle`** (as `install-infra-role`). Puts `dist.zip` at `apps/<appId>/latest/dist.zip` in the artifacts bucket. Skipped if no `zipBuffer`.
9. **`install_compute_stack`** (as `install-infra-role`). Runs an inline Pulumi program built from the manifest (`buildPulumiProgram`):
   ...
   - Lambda env always gets `STARKEEP_APP_ID`, `STARKEEP_STACK_PREFIX`, `STARKEEP_DSQL_HOSTNAME`, `STARKEEP_FILES_BUCKET`, plus the three cloud-client envs that activate cloud-mode `@starkeep/app-client`: `STARKEEP_APP_CLIENT_MODE=cloud`, `STARKEEP_CLOUD_DATA_BASE=<api-gateway-url>`, `STARKEEP_APP_CREDS_PARAMETER_NAME=/${stackPrefix}/app-creds/${appId}`. Any `env` keys the manifest left as empty strings (e.g. `STARKEEP_API_GATEWAY_URL`, `STARKEEP_USER_POOL_ID`, `STARKEEP_USER_POOL_CLIENT_ID`, `STARKEEP_IDENTITY_POOL_ID`) are filled by the installer from `~/.starkeep/config.json`.
   - Routes with `auth: "jwt"` (the default) attach the gateway's existing Cognito JWT authorizer; `auth: "public"` skips it entirely.
10. **`detach_temp_install_infra_policy`** (as Manager). Symmetric to step 7.
11. **`register_app`** — inserts one row into `shared.app_registry` keyed by app id (see "Registering" for what the row contains and how admin-web consumes it).
...
Once installed, an app runs as exactly one principal in the cloud: its IAM role `<stackPrefix>-app-<appId>-role`. Everything that role can do at runtime is the *intersection* of its inline policy, its permissions boundary, the bucket policy on the files bucket, the DSQL IAM-to-PG mapping, and the per-extension grants in `shared.access_grants`; on top of that, every broker call the app makes on its own behalf is HMAC-signed with the per-app secret, which the broker verifies before assuming the app's role. These layers don't overlap accidentally — each is set by a specific install step above, and each constrains a different surface:

- **HMAC secret in SSM** (`/${stackPrefix}/app-creds/${appId}`) — proves "this caller is `<appId>`" to the broker before any STS-assume happens. The same secret is mirrored to the local creds file and the local registry's `hmac_secret` column so the local sync supervisor signs with the matching value. A cross-app caller cannot forge the path-appId because the HMAC verifier requires `X-Starkeep-App-Id == path appId` *and* a valid signature.
- **Inline runtime policy** — built from `fileAccess` and `appSpecificSyncable`. Caps S3 access to the prefixes the app actually needs (`shared/<category>/<shard>/<hash>` for granted categories, `apps/<appId>/...` for app-private). Grants `sts:DbConnect` on `<pg-role>`, plus `ssm:GetParameter` on the app's own creds parameter and a service-scoped `kms:Decrypt` so the app's own Lambda can load its HMAC secret in cloud mode.
...
- **Environment.** Every handler is launched with `STARKEEP_APP_ID`, `STARKEEP_STACK_PREFIX`, `STARKEEP_DSQL_HOSTNAME`, `STARKEEP_FILES_BUCKET`, and the cloud-client envs `STARKEEP_APP_CLIENT_MODE=cloud`, `STARKEEP_CLOUD_DATA_BASE`, `STARKEEP_APP_CREDS_PARAMETER_NAME` so the app can find the platform and sign broker calls without configuration. Any additional `env` keys with empty-string values in the manifest are filled by the installer from `~/.starkeep/config.json` (currently: gateway URL, user pool id, user pool client id, identity pool id). This pattern means apps declare which platform variables they want without coupling to handler or app names.
...
- **Data access pattern.** With the 2026-06-10 change, cloud-served Lambdas no longer forward the inbound user `Authorization` header to the broker. Instead they import `@starkeep/app-client`, call `loadAppCredentialsAsync(appId)` (which in cloud mode fetches the HMAC secret from SSM via the Lambda's own IAM role), and call `signedFetch(creds, "/data/...")` etc. The HMAC handshake identifies the *app*; end-user identity is the app's business and is not propagated to the data plane. The Lambda's own role still technically permits direct DSQL and S3 access for what its manifest declared, but the broker remains the recommended single path because (a) the access-enforcer's per-extension check is finer than the IAM boundary, and (b) it leaves a single auditable surface.
...
- **Web tier.** The `static` handler runs the OpenNext build of `photos-web` with `auth: "public"` on `GET /` and `GET /{proxy+}`. The Next.js server inside is configured by the installer with `STARKEEP_API_GATEWAY_URL`, user-pool ids, identity-pool id, plus the cloud-client envs (`STARKEEP_APP_CLIENT_MODE=cloud`, `STARKEEP_CLOUD_DATA_BASE`, `STARKEEP_APP_CREDS_PARAMETER_NAME`). Its Next.js API routes (e.g. `/api/photos/captions/[id]`) call `await loadAppCredentialsAsync("photos")` to fetch the app's HMAC secret from SSM, then `signedFetch(creds, "/app-data/db/image_enriched", …)` to read/write captions through the broker. There is no embedded SDK in this handler — it is a true thin client, now signing as the app rather than forwarding a user JWT.
- **Compute tier (`resize-handler`).** Triggered by `POST /apps/photos/api/resize` with `auth: "jwt"`. The handler historically **forwarded the inbound `Authorization` header on every call to the broker** — fetching the source record, requesting a presigned URL for the original bytes, presigning a PUT for the resized bytes, registering the thumbnail record under the source's id as `parentId`, and writing dimensions into `image` metadata. With the HMAC migration the broker no longer reads the forwarded JWT; it expects each `/data/*` call to also carry HMAC headers. The resize Lambda still does no direct DSQL or S3 calls; everything rides the broker. (See Part 2 — the in-tree resize handler does not appear to have been migrated to `signedFetch` in this pass; broker calls from it will currently fail HMAC verification.)
...
6. **`delete_app_creds_parameter`** (as Manager). Deletes the SSM SecureString at `/${stackPrefix}/app-creds/${appId}`. Idempotent — `ParameterNotFound` is swallowed. Stragglers from interrupted runs are cleaned by `scripts/teardown-bootstrap.sh`, which walks `/${stackPrefix}/app-creds/*` at bootstrap teardown.
...
- **(Deferred with todo 39)** **HMAC signature does not bind method, path, or timestamp.** Input is `${appId}:` ++ body bytes. A signed POST is replayable against any other POST endpoint that accepts the same body, indefinitely. This is a meaningful weakening relative to the prior Cognito-JWT posture where tokens carried short expiries. The risk is bounded today (the secret is internal to the platform), but the missing bindings are worth surfacing for any future surface that accepts the same shape on multiple paths.
- **(Deferred with todo 42)** **Cloud-mode `loadAppCredentials` synchronous call returns `null` silently.** The sync function short-circuits to `null` when `STARKEEP_APP_CLIENT_MODE === "cloud"`. Existing call sites that don't migrate to the async variant will hit a `notInstalled()`-shaped 503 even though the secret is provisioned. The right end state is to remove the sync export entirely so this footgun cannot exist.
- **(Deferred with todo 16)** **HMAC secret cache on the broker has no invalidation hook.** `hmacSecretCache` holds each app's secret for 5 minutes; a quick uninstall + reinstall flips the SSM value but warm Lambdas continue to accept signatures under the prior secret until the cache expires. Symmetric to the existing STS-cred staleness issue and folded into the same todo.
