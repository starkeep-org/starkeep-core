...
import {
  ...
  stopById,
  type StartOutcome,
} from "../lib/daemon-control";
import { findApp } from "../lib/app-scan";
...
function startResponse(outcome: StartOutcome): Response {
...
export async function POST(req: Request) {
  ...
  if (action === "restart") {
    ...
    if (result === null) return Response.json({ restarted: false });
    if (!result.ok) return Response.json({ error: result.error }, { status: 500 });
    return Response.json({ restarted: true, ...result });
  }
  ...
  if (action === "start") {
    ...
    if (!found) {
      return Response.json({ error: `No manifest found for app '${id}' in configured app parent dirs` }, { status: 404 });
    }
