          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">In $/MTok</TableHead>
                <TableHead className="text-right">Out $/MTok</TableHead>
              ...
              {rows.map((row) => {
                ...
                return (
                  <TableRow key={row.modelId}>
                    ...
                    <TableCell className="text-right">{fmtPrice(e.inputPerMTok)}</TableCell>
                    <TableCell className="text-right">{fmtPrice(e.outputPerMTok)}</TableCell>
...
function seedDraft(row: ModelRow | null, isNew: boolean): Draft {
  ...
  return {
    ...
    overridePricing: o.inputPerMTok !== undefined,
    inValue: num(o.inputPerMTok ?? eff?.inputPerMTok),
    outValue: num(o.outputPerMTok ?? eff?.outputPerMTok),
...
function ModelEditDialog({
  ...
  const buildOverride = (): { override: ModelOverrideInput } | { error: string } => {
    ...
    if (draft.overridePricing) {
      const i = Number(draft.inValue);
      const o = Number(draft.outValue);
      if (!Number.isFinite(i) || !Number.isFinite(o) || i < 0 || o < 0) {
        return { error: "Input/output $/MTok must be non-negative numbers." };
      }
      override.inputPerMTok = i;
      override.outputPerMTok = o;
    }
    ...
    <Dialog open onOpenChange={(o) => { if (!o && !busy) onClose(); }}>
      <DialogContent className="max-w-lg">
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Pricing */}
          <OverrideField
            label="Token pricing ($/MTok)"
            enabled={draft.overridePricing}
            onToggle={(v) => set("overridePricing", v)}
            hint={platform ? `${inherited(platform.inputPerMTok)} in / ${platform.outputPerMTok ?? "—"} out` : undefined}
          >
            <div className="flex items-center gap-2">
              <Input className="w-28" type="number" min={0} step="0.01" placeholder="in"
                disabled={!draft.overridePricing} value={draft.inValue}
                onChange={(e) => set("inValue", e.currentTarget.value)} />
              <span className="text-muted-foreground text-sm">in</span>
              <Input className="w-28" type="number" min={0} step="0.01" placeholder="out"
                disabled={!draft.overridePricing} value={draft.outValue}
                onChange={(e) => set("outValue", e.currentTarget.value)} />
