...
import { useDataSource } from "../../../src/lib/data-source-context";
...
export default function FileBrowserPage() {
  ...
  const { data: typesData, loading, error } = useDataServer<{
    types: TypeSummary[];
    total: number;
  }>("/data/types", mode);
        ...
        <Paper p="lg" withBorder>
          ...
          <Table>
            <Table.Tbody>
              ...
              <Table.Tr>
                <Table.Td fw={600}>Owner</Table.Td>
                <Table.Td>
                  <Code fz="xs">{record.owner_id}</Code>
                </Table.Td>
              </Table.Tr>
              <Table.Tr>
                <Table.Td fw={600}>Version</Table.Td>
                <Table.Td>{record.version}</Table.Td>
              </Table.Tr>
              <Table.Tr>
              ...
              <Table.Tr>
                ...
                <Table.Td>{new Date(record.updated_at).toLocaleString()}</Table.Td>
              </Table.Tr>
              {record.mime_type && (
                <Table.Tr>
                  <Table.Td fw={600}>MIME Type</Table.Td>
                  <Table.Td>
                    <Code>{record.mime_type}</Code>
                  </Table.Td>
                </Table.Tr>
              )}
              {record.size_bytes != null && (
                <Table.Tr>
                  ...
                  <Table.Td>{formatSize(record.size_bytes)}</Table.Td>
                </Table.Tr>
              )}
              {record.content_hash && (
                <Table.Tr>
                  <Table.Td fw={600}>Content Hash</Table.Td>
                  <Table.Td>
                    <Code fz="xs">{record.content_hash}</Code>
                  </Table.Td>
                </Table.Tr>
              )}
              {record.object_storage_key && (
                <Table.Tr>
                  <Table.Td fw={600}>Storage Key</Table.Td>
                  <Table.Td>
                    <Code fz="xs">{record.object_storage_key}</Code>
                  </Table.Td>
                </Table.Tr>
              )}
            </Table.Tbody>
