...
import {
  Container, Title, Text, Paper, Stack, Group, Button, Alert, Loader,
} from "@mantine/core";
...
function CloudConfigSection() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      // Strip refresh token and credentials — export only contains static pool/bucket IDs
  ...
  async function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const required: (keyof CloudConfigExport)[] = [
        "stackPrefix", "s3Bucket", "s3Region", "auroraEndpoint", "cognitoConfig",
      ];
      ...
      // Read existing config to preserve the refresh token if present
  ...
  return (
    <Paper p="xl" withBorder>
      <Title order={3} mb="xs">Cloud Configuration</Title>
      <Text c="dimmed" size="sm" mb="md">
        Export your cloud config to share with another device (pool IDs + bucket +
        DSQL endpoint — no credentials or tokens). On the second device, import the
        file then sign in through the setup wizard.
      </Text>
      ...
      <Stack gap="sm">
        ...
        {exportSuccess && (
          <Alert color="green" title="Exported">
            starkeep-cloud-config.json downloaded — copy it to your other device and
            use "Import Cloud Config" there.
          </Alert>
        )}
...
function RedeploySection() {
  ...
  const handleRedeploy = useCallback(async () => {
    ...
    try {
      ...
      // Get fresh STS credentials via the stored refresh token
      ...
      if (!outputs["bucketName"] || !outputs["auroraHostname"]) {
        throw new Error(`Deployment outputs missing expected values. Parsed: ${JSON.stringify(outputs)}`);
      }

      // Write updated config with new infrastructure endpoints
  ...
  return (
    <Paper p="xl" withBorder>
      <Title order={3} mb="xs">Infrastructure</Title>
      <Text c="dimmed" size="sm" mb="md">
        Redeploy the remote Starkeep infrastructure (S3, Aurora DSQL, Lambda, API Gateway)
        using the current source. Updates cloud-config.json with any new endpoints on success.
      </Text>
