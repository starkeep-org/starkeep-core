...
- **User-Data-Owner role.** A reserved name with an empty trust policy — declared but unassumable. Reserved for a future Drive-style app; its existence is part of the namespace contract today.

### Permissions boundaries

A permissions boundary is the *ceiling* on what an identity can do, regardless of what policies are attached to it. Starkeep uses four:
...
- **Cloud-data-server is a broker, not a data owner.** Its own credentials never read or write shared data; every shared byte is attributable to the per-app role that the broker assumed for that specific request.
...
Sync from the local data server replays each record under its *originating* app's identity, not as a fallback identity, by re-assuming the origin app's role on the cloud side. Records whose origin app is uninstalled are rejected, never silently written under the broker's creds.

---

## The local data server

The local data server runs outside AWS and holds no AWS credentials. It mediates the local SDK's access to local SQLite + filesystem using the same SDK code paths the cloud uses, so a manifest grant that's wrong (or missing) produces the same denial in dev as in production. Its only interaction with the cloud is via the sync API, which is where the per-record origin-app re-attribution above kicks in.

This is why the local data server doesn't need its own AWS identity: it does not directly touch cloud data. The IAM identity for any cloud-side write is always assumed inside the cloud-data-server based on the record's origin app, not on who is calling the sync API.
