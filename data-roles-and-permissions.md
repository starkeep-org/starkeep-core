...
- **Foundational permissions boundary.** A wider ceiling used only by the cloud-data-server role. Permits the foundational provisioning actions (DSQL cluster admin, S3 bucket admin on the well-known files and billing bucket name patterns, the cost-and-usage-report (CUR) definition API, Lambda/API-Gateway admin scoped to cloud-data-server's own resources, and CloudFront admin for the platform distribution and its URL-signing key material) that the regular per-app boundary intentionally forbids. CloudFront is capped as `cloudfront:*` (a global service whose actions almost all lack resource-level ARNs), but that is only the *ceiling*: the specific verbs are granted by the ephemeral temp-install policy and detached after install, and the signing SecureString is written by Manager (not the CDS role, which stays read-only on SSM). A magic-string check in the installer routes only the cloud-data-server app id to this boundary, so a third-party app cannot opt into it.
...
1. The client (browser) reaches the shared API Gateway through the platform-owned CloudFront distribution, sending a Cognito JWT (identifies the user) and an app identification on the request (path or header, identifies the app). CloudFront is an **optimization layer, not a security boundary**: it edge-caches app static assets and passes API calls through to the gateway unchanged (forwarding all viewer headers except `Host`, so JWT and HMAC signatures survive intact). The gateway remains directly reachable — locking it to CloudFront-only access is not part of this design — so all authorization still happens at the gateway/handler exactly as described below; nothing may assume edge-level protection. (Server-to-server HMAC calls skip CloudFront and hit the gateway URL directly.)
...
**Shared file *bytes* are served via CloudFront signed URLs, not S3 presigned GETs.** The three shared file-url endpoints (`GET /data/records/:id/file-url`, the batch `POST /data/records/file-urls`, and the shared-prefix `GET /files/{key}/presign`) still run their existing authorization checks (per-type on the record-id routes, per-category on the raw-key route), but they mint a CloudFront signed URL against the platform distribution's `shared/*` behavior instead of presigning S3. This has one deliberate trust consequence: **the per-app IAM ceiling no longer applies to shared *reads*.** The CloudFront signing key is not category-scoped — whatever `shared/*` key the platform code signs, CloudFront serves — so the layered nested-ceiling property (IAM ⊇ PG GRANT ⊇ app-layer check) is reduced on this one path to two ceilings: the **structural** `shared/*` behavior scope (the distribution has no behavior for `apps/*` or any other prefix, and OAC opens the bucket only to the distribution, so the worst case of an app-layer bug is a cross-type leak *within shared data* — never private app files or other buckets) plus the **application-layer grant check**. Because the app-layer check is now the sole per-request line of defense here, all shared-read signing funnels through one **pre-sign revalidation chokepoint** (`signSharedCloudFrontUrl`) that re-parses the key, requires the `shared/` namespace, rejects path traversal, and re-checks the key's category against the caller's grants immediately before signing — no route reaches the signer around it. The signing private key lives in an SSM SecureString readable only by the cloud-data-server Lambda role (per-app roles cannot read it), so a malicious app cannot sign URLs itself; it can only call the platform endpoints, which enforce its install-time grants. **Writes and app-data reads are unchanged:** presigned PUT uploads and the `apps/<appId>/syncable/*` read path stay on S3 presign and remain fully IAM-bounded. If the layering ever needs restoring, the documented escape hatch is per-category key groups or reverting hot categories to S3 presign.
