...
/**
 * Generate a CloudFormation template for Immich photo management with OpenDAL
 */
function generateImmichTemplate(params: TemplateParams): string {
  return `AWSTemplateFormatVersion: '2010-09-09'
Description: Immich Photo Management with ECS, RDS PostgreSQL, and OpenDAL Storage - ${params.environment}
...
Parameters:
  ...
  DBPassword:
    ...
    MinLength: 8
    Default: ImmichPassword123

  ImmichVersion:
    Type: String
    Default: release
    Description: Immich version tag

  EC2InstanceType:
    Type: String
    Default: t3.large
    Description: EC2 instance type for Immich ECS hosts
...
Resources:
  ...
  # S3 Bucket for Photo Storage (via OpenDAL)
  PhotoStorageBucket:
    Type: AWS::S3::Bucket
    Properties:
      BucketName: !Sub '\${AWS::StackName}-\${Environment}-photos'
      VersioningConfiguration:
        Status: Enabled
      LifecycleConfiguration:
        Rules:
  ...
  DBParameterGroup:
    ...
    Properties:
      Description: Parameter group for Immich PostgreSQL
      Family: postgres16
      Parameters:
        rds.force_ssl: '0'
      Tags:
  ...
  DBSubnetGroup:
    ...
    Properties:
      DBSubnetGroupDescription: Subnet group for Immich database
      SubnetIds:
        - !Ref PrivateSubnet1
        - !Ref PrivateSubnet2
      Tags:
  ...
  LifecycleLambda:
    ...
    Properties:
      ...
      Environment:
        Variables:
          EIP_ALLOCATION_ID: !GetAtt ImmichEip.AllocationId
      Code:
        ZipFile: |
          ...
          exports.handler = async (event) => {
            ...
            try {
              ...
              console.error('Failed to update EIP association:', error);
            }
  ...
  # ECS Task Role (for S3 access via OpenDAL)
  ECSTaskRole:
    Type: AWS::IAM::Role
    Properties:
      AssumeRolePolicyDocument:
  ...
  # Immich Server Task Definition
  ImmichServerTaskDefinition:
    Type: AWS::ECS::TaskDefinition
    Properties:
      Family: !Sub '\${AWS::StackName}-immich-server'
      NetworkMode: host
      ...
      ContainerDefinitions:
        - Name: data-gateway
          Image: !Sub '\${AWS::AccountId}.dkr.ecr.\${AWS::Region}.amazonaws.com/starkeeper-gateway:latest'
          Essential: true
          PortMappings:
            - ContainerPort: 8080
              Protocol: tcp
          Environment:
            - Name: GATEWAY_BACKEND
              Value: s3
            - Name: GATEWAY_S3_BUCKET
              Value: !Ref PhotoStorageBucket
            - Name: GATEWAY_S3_REGION
              Value: !Ref AWS::Region
            - Name: GATEWAY_MOUNT_POINT
              Value: /mnt/data
            - Name: GATEWAY_MANAGEMENT_PORT
              Value: '8080'
            - Name: RUST_LOG
              Value: info,opendal=debug
          MountPoints:
        ...
        - Name: immich-ml
          ...
          LogConfiguration:
            ...
            Options:
              awslogs-group: !Ref LogGroup
              awslogs-region: !Ref AWS::Region
              awslogs-stream-prefix: immich-ml
        - Name: immich-server
          Image: !Sub 'ghcr.io/immich-app/immich-server:\${ImmichVersion}'
          Essential: true
          DependsOn:
            - ContainerName: data-gateway
              Condition: HEALTHY
          PortMappings:
            - ContainerPort: 2283
              Protocol: tcp
          ...
          LogConfiguration:
            ...
            Options:
              awslogs-group: !Ref LogGroup
              awslogs-region: !Ref AWS::Region
              awslogs-stream-prefix: immich-server
  Volumes:
    - Name: gateway-data
  ...
  ImmichServerService:
    ...
    DependsOn:
      - ECSAutoScalingGroup
    Properties:
      ServiceName: !Sub '\${AWS::StackName}-immich-server'
...
Outputs:
  ...
  ServiceName:
    Description: Immich ECS Service name
    Value: !GetAtt ImmichServerService.Name
    Export:
      Name: !Sub '\${AWS::StackName}-ServiceName'

  AccessInstructions:
    Description: How to access Immich
    Value: !Sub 'Access Immich at http://\${ImmichEip}:2283'

  ImmichElasticIp:
    Description: Elastic IP address for Immich
    Value: !Ref ImmichEip
  ...
  PhotoStorageBucketName:
    ...
    Value: !Ref PhotoStorageBucket
    Export:
      Name: !Sub '\${AWS::StackName}-PhotoStorageBucketName'
...
export function generateTemplate(input: GenerateTemplateInput): string {
  switch (input.appType) {
    ...
    case "immich":
      return generateImmichTemplate(input.params);
...
export function getAvailableAppTypes() {
  return [
    ...
    },
    {
      ...
      description: "Self-hosted photo & video management with ECS, RDS PostgreSQL, and S3 storage",
    },
    {
