  ...
  try {
    ...
    if (capabilityMatch && method === "POST") {
      ...
      const result = await handleCapabilityInvoke({
        ...
        imageInvoker: getBedrockImageInvoker(),
        // Sync-s3 image output (plan §3.8): the CDS writes the generated bytes to
        // the app's OWN syncable area UNDER THE APP ROLE (this `storage` is the
        // per-app-scoped adapter), one folder per invocation — so the capability
        // role never writes. The app then ingests the object as a normal record.
        writeSyncOutput: async (invocationId, images, contentType) => {
          const safe = invocationId.replace(/[^a-zA-Z0-9_-]/g, "-");
          const keyPrefix = `apps/${appId}/syncable/capability-image/${safe}`;
          const ext = contentType.split("/")[1] ?? "png";
          const keys: string[] = [];
          let totalBytes = 0;
          for (let i = 0; i < images.length; i++) {
            const key = `${keyPrefix}/image-${i}.${ext}`;
            await storage.put(key, images[i]!, { contentType });
            keys.push(key);
            totalBytes += images[i]!.byteLength;
          }
          return { bucket: s3Bucket, keyPrefix, keys, totalBytes };
        },
        region,
